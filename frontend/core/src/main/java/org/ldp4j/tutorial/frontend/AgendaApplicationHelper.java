...
final class AgendaApplicationHelper {
	...
	private static Logger LOGGER=LoggerFactory.getLogger(AgendaApplicationHelper.class);

	private static final Map<String,AtomicLong> COUNTERS=
		ImmutableMap.
			<String,AtomicLong>builder().
				put(PersonHandler.ID,new AtomicLong()).
				put(ContactContainerHandler.ID,new AtomicLong()).
				put(ContactHandler.ID,new AtomicLong()).
				build();
	...
	static Name<?> nextName(String templateId) {
		long nextId = COUNTERS.get(templateId).getAndIncrement();
		return
			NamingScheme.
				getDefault().
					name(
						templateId,
						Long.toHexString(nextId).toUpperCase(Locale.ENGLISH));
	}
